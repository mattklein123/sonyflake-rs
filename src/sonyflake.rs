...
pub(crate) fn default_start_time() -> OffsetDateTime {
    datetime!(2014-09-01 00:00:00 UTC)
}

/// Return the lowest ID generated by a default-epoch Sonyflake at or after `timestamp`.
///
/// Timestamps before the default epoch saturate at zero. Timestamps beyond the representable ID
/// range saturate at `u64::MAX`.
pub fn minimum_for_timestamp(timestamp: OffsetDateTime) -> u64 {
    minimum_for_time(timestamp, to_sonyflake_time(default_start_time()))
}
...
impl Sonyflake {
    ...
    pub fn min_sonyflake_for_time(&self, time: OffsetDateTime) -> u64 {
        minimum_for_time(time, self.0.start_time)
    }
...
fn minimum_for_time(timestamp: OffsetDateTime, start_time: i64) -> u64 {
    let elapsed_time = to_sonyflake_time(timestamp)
        .saturating_sub(start_time)
        .max(0);
    u64::try_from(elapsed_time)
        .unwrap_or(u64::MAX)
        .checked_shl((BIT_LEN_SEQUENCE + BIT_LEN_MACHINE_ID) as u32)
        .unwrap_or(u64::MAX)
}
